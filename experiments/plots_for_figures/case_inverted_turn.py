...
import numpy as np
from arena import plot_analytical_trajectory, get_corner_point, shrink_corridor_list, compute_end_pose, compute_start_pose, MotionPlanner, CurvilinearArcUnicycle, CorridorWorld, Unicycle, get_corridor_from_vector, plot_corridors
from math import sin, cos, pi, sqrt, asin, atan2
...
figure_corridors = plot_corridors(corridor_list = corridor_list, color = 'k', linestyle = '-', linewidth = 1, figure = None, plot_vectors = False)
plt.show(block = True)
...
mp = MotionPlanner(vehicle, corridor_list, end_pose=final_pose)
...
plot_analytical_trajectory(analytical_trajectory, figure = figure1)
