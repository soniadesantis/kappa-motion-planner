...
import numpy as np
from arena import plot_velocity_profiles, shrink_corridor_list, compute_end_pose, compute_start_pose, MotionPlanner, CurvilinearArcUnicycle, CorridorWorld, Unicycle, get_corridor_from_vector, plot_corridors
from math import sin, cos, pi, sqrt, asin, atan2
...
## Figure 1: corridor sequence
figure1 = plot_corridors(corridor_list = corridor_list, color = 'k', linestyle = '-', linewidth = 1, figure = None, plot_vectors = False)
plt.axis('off')
plt.tight_layout()
plt.savefig("ugm_example1.svg", format='svg', bbox_inches='tight', pad_inches=0, transparent=True)

## Figure 2: corridor sequence + initial and final pose
...
plt.axis('off')
plt.tight_layout()
plt.savefig("ugm_example2.svg", format='svg', bbox_inches='tight', pad_inches=0, transparent=True)
...
plt.axis('off')
plt.tight_layout()
plt.savefig("ugm_example3.svg", format='svg', bbox_inches='tight', pad_inches=0, transparent=True)

plot_velocity_profiles(trajectory, vehicle)
plt.savefig("ugm_example4.svg", format='svg', bbox_inches='tight', pad_inches=0, transparent=True)
